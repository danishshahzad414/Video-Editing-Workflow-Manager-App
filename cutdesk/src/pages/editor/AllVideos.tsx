      ...
      <div className="fade-in space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {PIPELINE_STAGES.map(s => (
                <button key={s} onClick={() => toggleFilter(statusFilter, s, setStatusFilter)} className="text-xs px-3 py-1 rounded-full transition-all" style={{ background: statusFilter.includes(s) ? stageColor(s) : '#F1F5F9', color: statusFilter.includes(s) ? '#fff' : '#475569', fontFamily: 'Plus Jakarta Sans', fontWeight: 600, border: '1px solid #E2E8F0' }}>{s}</button>
              ))}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                {['Urgent', 'Normal', 'Low'].map(p => (
                  <button key={p} onClick={() => toggleFilter(priorityFilter, p, setPriorityFilter)} className="text-xs px-3 py-1 rounded-full transition-all" style={{ background: priorityFilter.includes(p) ? '#0284C7' : '#F1F5F9', color: priorityFilter.includes(p) ? '#fff' : '#475569', fontFamily: 'Plus Jakarta Sans', fontWeight: 600, border: '1px solid #E2E8F0' }}>{p}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="cd-card overflow-hidden">
          <div className="px-5 py-3" style={{ borderBottom: '1px solid #E2E8F0' }}>
            <p className="text-xs" style={{ fontFamily: 'Plus Jakarta Sans', color: '#94A3B8' }}>{filtered.length} videos</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px]">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={7} className="px-4 py-12 text-center" style={{ fontFamily: 'Plus Jakarta Sans', color: '#94A3B8' }}>No videos match your filters</td></tr>
                ) : filtered.map((v, i) => (
                  <tr
                    key={v.id}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-xs text-center" style={{ fontFamily: 'Plus Jakarta Sans' }}>
