      ...
      <div className="fade-in space-y-5">
        ...
        <div className="cd-card p-4 flex items-center gap-6 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm" style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 600, color: '#0F172A' }}>This week:</span>
            <span className="text-sm" style={{ fontFamily: 'Plus Jakarta Sans', color: '#475569' }}>{queueVideos.length} videos in queue</span>
          </div>
          <div className="h-4 w-px" style={{ background: '#E2E8F0' }} />
          <span className="text-sm" style={{ fontFamily: 'Plus Jakarta Sans', color: '#475569' }}>Est. time: {estTotalHours}h</span>
          <div className="h-4 w-px" style={{ background: '#E2E8F0' }} />
          <div className="flex items-center gap-2">
            <span className="text-sm" style={{ fontFamily: 'Plus Jakarta Sans', color: '#475569' }}>Available:</span>
            <input
              ...
              style={{ background: '#F1F5F9', border: '1.5px solid #E2E8F0', color: '#0F172A', height: 32, fontFamily: 'Plus Jakarta Sans', fontWeight: 600 }}
            />
            <span className="text-sm" style={{ fontFamily: 'Plus Jakarta Sans', color: '#475569' }}>h</span>
          </div>
          <div className="h-4 w-px" style={{ background: '#E2E8F0' }} />
          <span className={`text-sm font-semibold flex items-center gap-1.5 ${isOverloaded ? 'text-amber-500' : 'text-emerald-600'}`} style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 600 }}>
            ...
            return (
              <div key={i} className="cd-card overflow-hidden">
                <div className="px-3 py-2" style={{ borderBottom: '1px solid #F1F5F9', background: isHeavy ? '#FFFBEB' : '#F8FAFC' }}>
                  <p className="text-xs font-bold" style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, color: isHeavy ? '#B45309' : '#475569' }}>
                    {DAYS[i]} {isHeavy && '⚠'}
                  </p>
                ...
                <div className="p-2 space-y-1.5">
                  {dayVideos.map(v => (
                    <div key={v.id} className="px-2 py-1 rounded text-xs truncate" style={{ background: '#EFF6FF', color: '#0284C7', fontFamily: 'Plus Jakarta Sans', fontWeight: 500 }}>
        ...
        {unscheduled.length > 0 && (
          <div className="cd-card overflow-hidden">
            <div className="px-5 py-3" style={{ borderBottom: '1px solid #E2E8F0' }}>
              <h2 className="text-sm" style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, color: '#0F172A' }}>Unscheduled Videos ({unscheduled.length})</h2>
            </div>
            <div className="p-4 space-y-2">
              {unscheduled.map(v => (
                <div key={v.id} className="flex items-center gap-4">
                  <p className="flex-1 text-sm truncate" style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 500, color: '#0F172A' }}>{v.title}</p>
                  <div className="flex items-center gap-2">
                    <input
                      type="date"
