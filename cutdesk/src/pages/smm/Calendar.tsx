        ...
        {days.map(day => {
          ...
          return (
            <div key={str} className="cd-card min-h-32">
              <div className="px-2 py-1.5 border-b border-[rgba(0,162,207,0.1)]">
                <p className="text-white/70 text-xs font-semibold" style={{ fontFamily: 'Poppins', fontWeight: 700 }}>{format(day, 'EEE')}</p>
                <p className="text-white/30 text-[10px]" style={{ fontFamily: 'Poppins' }}>{format(day, 'MM/dd')}</p>
              </div>
        ...
        {days.map(day => {
          ...
          return (
            <div key={str} className="cd-card p-1.5 min-h-16">
              <p className="text-white/40 text-[10px] mb-1" style={{ fontFamily: 'Poppins' }}>{format(day, 'd')}</p>
              {vids.map(v => (
      ...
      <div className="fade-in space-y-4">
        <div className="flex gap-2">
          {(['weekly', 'monthly'] as View[]).map(v => (
            <button key={v} onClick={() => setView(v)} className="px-4 py-2 rounded-lg text-xs capitalize transition-all" style={{ background: view === v ? '#00A2CF' : 'rgba(255,255,255,0.08)', color: '#fff', fontFamily: 'Poppins', fontWeight: 600 }}>{v}</button>
          ))}
        </div>
